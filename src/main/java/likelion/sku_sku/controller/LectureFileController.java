...
import likelion.sku_sku.dto.LectureFileDTO;
...
public class LectureFileController {
    ...
    @PostMapping("/upload")
    public ResponseEntity<List<LectureFile>> uploadFiles(@ModelAttribute LectureFileDTO.uploadLectureFileRequest lectureFileDto) throws IOException {
        List<MultipartFile> files = lectureFileDto.getFiles();
        List<LectureFile> savedFiles = lectureFileService.saveFiles(files);
