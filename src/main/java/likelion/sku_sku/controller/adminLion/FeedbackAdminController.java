...
import likelion.sku_sku.service.SubmitFeedbackService;
...
public class FeedbackAdminController {
    ...
    @PostMapping("/add")
    public ResponseEntity<Map<String, Object>> setPassStatusFeedback(@RequestBody FeedBackPassStatus request) {
        Map<String, Object> response = submitFeedbackService.addFeedbackPassStatus
                (request.getSubmitAssignmentId(), request.getPassNonePass(), request.getContent());
        return ResponseEntity.status(HttpStatus.CREATED).body(response);
    }

    @Operation(summary = "(민규) 제출한 과제 상태 변경 또는 피드백 수정", description = "Headers에 Bearer token 필요, body에 json 형태로 피드백 id, 수정하고 싶은 값 필요",
            responses = {@ApiResponse(responseCode = "201", description = "수정 성공"),
                    @ApiResponse(responseCode = "404", description = "그 id에 해당하는 값 없음")})
    @PutMapping("/update")
    public ResponseEntity<Map<String, Object>> updateFeedback(@RequestBody UpdateFeedBackPassStatust request) {
        Map<String, Object> response = submitFeedbackService.updateFeedbackPassStatus(request.getFeedBackId(), request.getPassNonePass(), request.getContent());
        return ResponseEntity.status(HttpStatus.CREATED).body(response);
    }
}
